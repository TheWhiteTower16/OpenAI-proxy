...
export default {
  ...
  run: function (value, request, response, config, stats) {
    ...
    if (
      ["/v1/completions", "/v1/chat/completions"].indexOf(stats.endpoint) === -1
    )
      return;
    ...
    if (response.choices) {
      ...
      response.choices.forEach(function (c) {
        if (c.text) {
          const promptToCheck = getSubstringBetweenDelimiters(
            request.prompt,
            config.PROMPT_REFLECTION_DELIMETER,
          );
          if (promptToCheck) {
            const reg = new RegExp(promptToCheck, "ig");
            if (c.text.match(reg)) {
              if (value == "redact")
                c.text = c.text.replace(reg, config.REDACTION_STRING);
            ...
            if (r.role == "system") {
              ...
              if (promptToCheck) {
                ...
                if (c.message.content.match(reg)) {
                  if (value == "redact")
                    c.message.content = c.message.content.replace(
                      reg,
                      config.REDACTION_STRING,
                    );
                  if (value == "block") stats.error = true;
                  flagged = true;
                }
      ...
      if (flagged) {
        stats.flags.push({
          ...
          description:
            "The response contained a reflection of the original prompt",
        });
      }
    }
  },
